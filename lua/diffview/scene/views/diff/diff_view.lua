...
local stack_blessed_ok, stack_blessed = pcall(require, "custom.stack-blessed")
...
DiffView.update_files = debounce.debounce_trailing(
  ...
  async.wrap(function(self, callback)
    ...
    -- Populate blessed review status
    if stack_blessed_ok and self.rev_arg then
      -- rev_arg is "parent..branch" — extract branch name
      local branch = self.rev_arg:match("%.%.(.+)$")
      if branch then
        for _, file in self.files:iter() do
          file.blessed_status = stack_blessed.file_status(branch, file.path)
        end
      end
    end
